...
>(
  ...
  return {
    ...
    endpoints: {
      ...
      onboardingStatus: createAuthEndpoint(
        ...
        async (ctx): Promise<OnboardingStatusResponse> => {
          ...
          const [shouldOnboard, currentStep, completedSteps] =
            await Promise.all([
          ...
          const effectiveCurrentStep =
            currentStep ?? getFirstIncompleteStep(stepOrder, completedSteps);
      ...
      after: [
        ...
        // After sign-up: set shouldOnboard to true
        {
          ...
          handler: createAuthMiddleware(async (ctx) => {
            ...
            // Preserve Better Auth's signup response and refresh its cached user fields.
            const fresh = await ctx.context.internalAdapter.findSession(
              newSession.session.token,
            );
            if (fresh) await setSessionCookie(ctx, fresh);
          }),
        },
      ],
    },
    ...
    schema: !options.secondaryStorage
      ? mergeSchema(schema, undefined)
      : undefined,

    $ERROR_CODES: Object.fromEntries(
      Object.entries(ONBOARDING_ERROR_CODES).map(([code, message]) => [
        code,
        { code, message },
      ]),
    ),
